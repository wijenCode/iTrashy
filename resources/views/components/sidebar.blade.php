<div id="menu" class="fixed top-0 left-0 w-60 h-full bg-white shadow-lg transform -translate-x-full transition-transform duration-300 z-50 lg:translate-x-0 lg:static lg:w-[230px]">
    ...
    <nav class="p-5 mt-5 space-y-8">
        ...
        @if(auth()->check() && auth()->user()->role === 'user')
            ...
            <!-- Riwayat Menu -->
            <a href="{{ route('setor.sampah.history') }}" class="nav-link flex items-center space-x-4 hover:bg-gray-200 hover:ml-[-20px] hover:pl-[30px] hover:pr-[5px] hover:py-[10px] hover:font-medium hover:rounded-r-xl transition-all duration-300">
                <img src="{{ asset('storage/images/riwayat.png') }}" alt="Riwayat Icon" class="w-6 h-6">
                <span>Riwayat</span>
            </a>
        @endif

        <!-- Menu untuk Driver (role driver) -->
        @if(auth()->check() && auth()->user()->role === 'driver')
            <!-- Ambil Sampah Menu -->
            <a href="{{ route('driver.ambil.sampah') }}" class="nav-link flex items-center space-x-4 hover:bg-gray-200 hover:ml-[-20px] hover:pl-[30px] hover:pr-[5px] hover:py-[10px] hover:font-medium hover:rounded-r-xl transition-all duration-300">
                <img src="{{ asset('storage/images/setor.png') }}" alt="Ambil Sampah Icon" class="w-6 h-6">
                <span>Ambil Sampah</span>
            </a>

            <!-- Penjemputan Saya Menu -->
            <a href="{{ route('driver.penjemputan.saya') }}" class="nav-link flex items-center space-x-4 hover:bg-gray-200 hover:ml-[-20px] hover:pl-[30px] hover:pr-[5px] hover:py-[10px] hover:font-medium hover:rounded-r-xl transition-all duration-300">
                <img src="{{ asset('storage/images/setor.png') }}" alt="Penjemputan Saya Icon" class="w-6 h-6">
                <span>Penjemputan Saya</span>
            </a>
        @endif
    </nav>
</div>
