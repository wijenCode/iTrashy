...
<div class="min-h-screen bg-gray-50 flex flex-col lg:flex-row gap-6 p-6">
        ...
        <div class="bg-white rounded-lg shadow-lg p-6">
            <div class="flex items-center mb-4">
                <div class="bg-blue-500 text-white px-4 py-2 rounded-tl-lg rounded-br-lg font-bold text-lg flex items-center">
                    <img src="{{ asset('assets/icon/poin logo.png') }}" class="w-6 h-6 mr-2"> Trash Poin <span class="ml-2">{{ number_format($voucher->poin) }}</span>
                </div>
            </div>
            <img src="{{ asset('assets/image/' . $voucher->gambar) }}" alt="Voucher Image" class="w-full h-40 object-cover rounded-lg mb-4">
            <h2 class="text-xl font-bold mb-2">Voucher {{ $voucher->nama_voucher }}</h2>
            <p class="text-gray-700 mb-2">Berlaku hingga {{ \Carbon\Carbon::parse($voucher->masa_berlaku)->translatedFormat('d F Y') }}</p>
            <p class="text-gray-600 mb-4">{{ $voucher->deskripsi }}</p>
            <div class="bg-gray-100 rounded-lg p-4 mb-4">
                <h4 class="font-semibold mb-2">Rincian Voucher</h4>
                <ul class="text-sm text-gray-700 list-disc pl-5 space-y-1">
                    @foreach($voucher->rincian as $rincian)
                        <li>{{ $rincian }}</li>
                    @endforeach
                </ul>
            </div>
            <div class="flex justify-between items-center mb-4">
                ...
                <span class="font-bold text-blue-600 text-lg">{{ number_format($voucher->poin) }}</span>
            </div>
            <form action="{{ route('voucher.tukar', $voucher->id) }}" method="POST">
                @csrf
    ...
    <div class="flex-1 max-w-md mx-auto lg:mx-0">
        <h3 class="text-xl font-bold mb-4">Voucher lainnya</h3>
        <div class="space-y-4">
            @foreach($vouchers as $item)
                @if($item->id != $voucher->id)
                <a href="{{ route('voucher.detail', $item->id) }}" class="block bg-white rounded-lg shadow-md p-4 hover:bg-blue-50 transition">
                    ...
                    <div class="flex items-center gap-3">
                        <img src="{{ asset('assets/image/' . $item->gambar) }}" alt="Voucher Image" class="w-16 h-12 object-cover rounded-lg">
                        <div>
                            <h4 class="font-semibold">{{ $item->nama_voucher }}</h4>
