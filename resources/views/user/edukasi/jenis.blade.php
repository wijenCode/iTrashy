...
<div class="container mx-auto p-4">
    ...
    <nav class="flex mb-5" aria-label="Breadcrumb">
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li>
                <div class="flex items-center">
                    <i class="fas fa-chevron-right text-gray-400 mx-2"></i>
                    <span class="text-gray-500">{{ ucfirst($jenis) }}</span>
                </div>
            </li>
        </ol>
    </nav>
    ...
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-800 mb-4 flex items-center">
            <i class="fas fa-{{ $jenis == 'video' ? 'video' : ($jenis == 'infografis' ? 'image' : 'newspaper') }} mr-3 text-{{ $jenis == 'video' ? 'red' : 'blue' }}-500"></i>
            {{ ucfirst($jenis) }}
        </h1>
        
        <!-- Category Pills -->
        <div class="flex flex-wrap gap-2">
            @foreach(['kesehatan', 'pendidikan', 'lingkungan'] as $kategori)
                <a href="{{ route('edukasi.kategori', $kategori) }}" 
                   class="px-4 py-2 rounded-full text-sm font-medium {{ request()->is("edukasi/kategori/$kategori") ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300' }} flex items-center">
                    {{ ucfirst($kategori) }}
                    @if(isset($kategoriCounts[$kategori]))
                        <span class="ml-2 px-2 py-0.5 bg-{{ request()->is("edukasi/kategori/$kategori") ? 'white text-blue-500' : 'gray-300' }} rounded-full text-xs">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @forelse($edukasi as $konten)
            <div class="bg-white rounded-xl shadow-lg overflow-hidden transform transition duration-300 hover:scale-105 hover:shadow-2xl">
                <a href="{{ route('edukasi.show', $konten->id) }}" class="block">
                        ...
                        <div class="relative h-48 overflow-hidden">
                            ...
                            <div class="absolute bottom-0 left-0 right-0 px-4 py-2 bg-gradient-to-t from-black to-transparent">
                                <div class="text-white text-sm">
                                    <span class="bg-{{ $jenis == 'video' ? 'red' : 'blue' }}-500 text-white text-xs px-2 py-1 rounded-full">
                    ...
                    @endif
                    <div class="p-5">
                        <h4 class="font-bold text-lg mb-3 text-gray-800 line-clamp-2 hover:text-{{ $jenis == 'video' ? 'red' : 'blue' }}-600 transition duration-200">
                            {{ $konten->judul_konten }}
                        </h4>
                        <div class="flex items-center justify-between text-sm text-gray-600">
                            <div class="flex items-center space-x-4">
                                <span class="flex items-center">
                                    <i class="fas fa-folder text-{{ $jenis == 'video' ? 'red' : 'blue' }}-500 mr-2"></i>
                                    {{ ucfirst($konten->kategori) }}
                                </span>
                                <span class="flex items-center">
                                    <i class="fas fa-calendar text-{{ $jenis == 'video' ? 'red' : 'blue' }}-500 mr-2"></i>
                                    {{ \Carbon\Carbon::parse($konten->created_at)->format('d M Y') }}
                                </span>
                            </div>
                            <span class="inline-flex items-center text-{{ $jenis == 'video' ? 'red' : 'blue' }}-500 group">
                                {{ $jenis == 'video' ? 'Tonton' : 'Baca' }}
        ...
        @empty
            <div class="col-span-3 text-center py-10">
                ...
                <p class="text-gray-500">Tidak ada konten untuk jenis ini.</p>
            </div>
        @endforelse
    </div>
